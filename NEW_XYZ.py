...
def replace_none_z_values(coordinates):
    ...
    for g_code, x, y, z, e in coordinates:
        ...
        z = z or current_z
        if float(z) != float(current_z):  # 检测Z值变化
...
def remove_single_data_between_empty_lines(coordinates):
    """
    删除两个空行之间只有一条数据的情况，包括删除该数据和一个空行。
    """
    cleaned_coordinates = []
    buffer = []  # 临时存储当前分段的数据
...
def process_gcode(gcode_file_path, output_file_path, output_box):
    ...
    save_to_txt(cleaned_coordinates, output_file_path)
    output_box.delete(1.0, tk.END)  # 清空输出框内容
    for line in cleaned_coordinates:
        if line is None:
